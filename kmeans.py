...
st.set_page_config(
    page_title="KMeans Step-by-Step Visualizer",
    layout="wide"
)
...
def get_random_centroids(k):
    """
    Randomly pick k distinct points from the active dataset
    (FIXED_POINTS[:15*k]) as starting centroids.
...
# =========================================================
# K-MEANS LOGIC  (manual, step-by-step — no sklearn)
# =========================================================
...
def move_centroids(points, labels, k, current_centroids):
    ...
    Falls back to the current position if a cluster is empty.
    (current_centroids passed in explicitly to avoid scoping issues.)
    """
    new_centroids = []
    for i in range(k):
        cluster_pts = points[labels == i]
        if len(cluster_pts) == 0:
            new_centroids.append(current_centroids[i])   # keep in place if empty
        else:
...
def build_steps(k, init_c):
    """
    Returns a list of step dicts, each describing one stage of the algorithm.
    init_c : np.array of shape (k, 2) — randomly chosen starting centroids.
    """
    pts = FIXED_POINTS[: 15 * k]
    steps = []
...
# =========================================================
# SESSION STATE  (persists current step across rerenders)
# =========================================================
...
# Generate random centroids once on first load; store them so they
# don't change every time Streamlit reruns (e.g. on button click).
...
# k changed → new random centroids + reset step
if k != st.session_state.k:
    st.session_state.k = k
    st.session_state.step_index = 0
    st.session_state.initial_centroids = get_random_centroids(k)

# Reset to step 1 but keep same centroids
if st.sidebar.button("↺ Reset to Step 1"):
    st.session_state.step_index = 0

# Pick brand-new random centroids and restart
...
# =========================================================
# BUILD STEPS  (uses the stored random centroids)
# =========================================================
...
# =========================================================
# STAGE PROGRESS BAR  (top of page)
# =========================================================
...
# =========================================================
# MAIN LAYOUT: CHART  |  EXPLANATION PANEL
# =========================================================
...
with chart_col:
    ...
    # Draw movement arrows for 'move' phase
    ...
    # Data points (one trace per cluster for legend)
...
# ── Right panel: explanation + centroid table ─────────────────────────────────
with panel_col:
